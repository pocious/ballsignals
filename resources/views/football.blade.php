...
@section('title', 'Football Tips — BallSignals')
@section('meta_description', 'Free daily football betting tips and predictions. Premier League, Champions League, La Liga, Serie A and more.')
...
<div class="max-w-2xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="mb-4">
        <div class="flex items-center justify-between mb-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="#football-results"
                   class="px-3 py-1.5 rounded-full bg-green-600 hover:bg-green-700 text-white font-bold text-xs transition-colors shadow-sm shadow-green-600/30 flex items-center gap-1">
    ...
    <div class="flex items-center gap-1.5 text-[10px] font-bold">
        ...
        <a href="{{ route('football', array_merge(request()->query(), ['sort' => $val])) }}"
           class="px-2.5 py-1 rounded-full transition-colors
                  {{ $selectedSort === $val ? 'bg-green-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-green-600/10 hover:text-green-600' }}">
            {{ $label }}
        </a>
        @endforeach
    </div>
    ...
    @if($tipsByDate->isEmpty())
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 py-16 text-center">
            ...
            <p class="font-semibold text-gray-700 dark:text-gray-300">No football tips right now</p>
            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">
                @if($selectedLeague)
                    No {{ $selectedLeague }} tips found.
                    <a href="{{ route('football') }}" class="text-green-600 dark:text-green-400 hover:underline ml-1">Clear filter</a>
                @else
                    Tips will appear here once matches are scheduled.
                @endif
            </p>
        </div>
    @else
                    ...
                    <div class="rounded-2xl overflow-hidden border {{ $isFinished ? 'border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 opacity-75' : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900' }}">
                        ...
                        <div class="grid grid-cols-3 overflow-hidden rounded-b-2xl border-t border-gray-100 dark:border-gray-800">
                            <a href="{{ route('tips.show', $tip) }}"
                               class="flex items-center justify-center gap-1 text-[10px] font-bold py-2 bg-green-600/10 hover:bg-green-600/20 text-green-700 dark:text-green-400 transition-colors border-r border-gray-100 dark:border-gray-800">
    ...
    {{-- VIP Premium Football Tips --}}
    @if($premiumTips->isNotEmpty())
    <div class="mt-8">
        <div class="flex items-center gap-2 mb-3 px-1">
            <div class="w-1 h-5 rounded-full bg-yellow-400"></div>
            <h2 class="text-sm font-bold uppercase tracking-widest text-yellow-600 dark:text-yellow-400">VIP Football Picks</h2>
            <span class="text-[10px] font-black text-black bg-yellow-400 px-1.5 py-0.5 rounded uppercase">PRO</span>
            <span class="text-xs text-gray-400 dark:text-gray-600 ml-auto">{{ $premiumTips->flatten()->count() }} tip{{ $premiumTips->flatten()->count() !== 1 ? 's' : '' }}</span>
        </div>

        @if($canSeePremium)
            <div class="space-y-3">
                @foreach($premiumTips->flatten() as $tip)
                    ...
                    <div class="px-4 pt-2.5 pb-2">
                        <div class="flex items-center justify-between gap-2 mb-1">
                            ...
                            <div class="flex items-center gap-1.5 flex-shrink-0">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1 text-[8px] px-1.5 py-px rounded-full font-bold bg-yellow-400/20 text-yellow-500 border border-yellow-400/40">Pending</span>
                                @endif
                            </div>
                        </div>
                        <div class="flex items-center justify-between gap-2">
            ...
            <div class="rounded-2xl overflow-hidden border border-yellow-300 dark:border-yellow-700/50" style="background:#0a0f1a;">
                ...
                <div class="px-4 py-3 {{ !$loop->last ? 'border-b border-yellow-400/10' : '' }}">
                    <div class="flex items-center justify-between gap-2 mb-1.5">
                        ...
                        <span class="inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full font-bold flex-shrink-0 bg-green-500/10 text-green-400 border border-green-500/20">
                            <span class="relative flex h-1.5 w-1.5">
        ...
        <div class="space-y-2">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 px-4 py-2.5 flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span class="text-[10px] font-semibold text-gray-500 dark:text-gray-400">@ {{ number_format($tip->odds, 2) }}</span>
                    @endif
                    <span class="text-[10px] font-black px-2 py-0.5 rounded-full text-white" style="background:#16a34a;">
                        {{ $tip->prediction }}
                    </span>
                    @if($tip->status === 'won')

...
<div class="max-w-2xl mx-auto px-4 py-6 space-y-6">
    ...
    @endphp
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 py-10 px-5 text-center">
            ...
            <p class="font-semibold text-gray-700 dark:text-gray-300">No basketball tips right now</p>
            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1 mb-6">Most leagues are on a break — tips appear as soon as matches are scheduled.</p>
        ...
        <div class="mt-5">
            ...
            <div class="space-y-2">
                ...
                @endforeach
                @php $totalSport = \App\Models\BettingTip::where('sport', $s['db'])->where('match_time', '>=', today()->startOfDay())->where('match_time', '<=', today()->addDays(7)->endOfDay())->where('is_premium', false)->count(); @endphp
                @if($totalSport > 5)
                <a href="{{ $s['route'] }}" class="flex items-center justify-center gap-1 py-2 text-[11px] font-bold text-orange-500 hover:text-orange-400 transition-colors">
                    View all {{ $totalSport }} {{ $s['name'] }} tips →
                </a>
                @endif
            </div>
        </div>
